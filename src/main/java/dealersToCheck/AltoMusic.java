...
public class AltoMusic extends CheckDealer {
	...
	public static List<List<Object>> getReportPerURL(String dealer, String category, String pageNumber,String url, int startingPosition, int nPosPerCat, String country) throws IOException, InterruptedException{
		...
		Elements devices = doc.select("div[class=row big_with_description]");
		Elements boxes = devices.select("div[class=span3 product]");
		...
		for (int i = 0; i<boxes.size(); i++){
			try{
	    		...
	    		List<String> brandName = splitBrandName(boxes.get(i).select("h3[class=product-name]").first().text());	
	    		...
	    		String getPrice = boxes.get(i).select("span[class=price]").first().text();
	    		String price="";
