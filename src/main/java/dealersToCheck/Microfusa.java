...
public class Microfusa extends CheckDealer {
	...
	public static List<List<Object>> getReportPerURL(String dealer, String category, String pageNumber,String url, int startingPosition, int nPosPerCat, String country) throws IOException, InterruptedException{
		...
		Document doc = Microfusa.connect(url);
		Elements devBox = doc.select("ul[class=products-grid two_columns_5]");
		Elements devices = devBox.select("li[class=item]");
		...
		for (int i = 0; i<devices.size(); i++){
			try{
	    		...
	    		List<String> brandName = splitBrandName(devices.get(i).select("a").attr("title"));	
	    		...
	    		String price="";
	    		try{	    			
					price=devices.get(i).select("div[class=grid_price]").text();
					price = String.valueOf(priceEditor(price.substring(0, 5)));
	    		} catch(Exception e){
